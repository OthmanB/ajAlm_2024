...
def nice_hist(samples, stats, ax=None, intervals=[True,True], binning=30, color=['black', 'gray', 'darkgray'], alpha=None, rotate=False, max_norm=None):
	...
	if ax == None:
		fig_1d, ax = plt.subplots(1, figsize=(12, 6), num=1, clear=True)
	if max_norm == None:
		...
		fig_1d, ax_dummy = plt.subplots(1, figsize=(12, 6), num=2, clear=True)
	...
	if rotate == False:
		try:
			...
			print("Warning: Could not add lines and fills the pdf plot!")
	else:
		try:
			if intervals[1] == True:
...
def nice_hist_matrix(samples, stats, labels, binning=30, posplots=None, file_out='plots_pdfs.jpg', extra_data=[]):
	...
	fig_2d, axes = plt.subplots(dim, dim, figsize=(12, 6), num=1, clear=True)#, subplot_kw={'xticks': [], 'yticks': []})
	...
	for i in range(0,dim-1):
		...
		axes[0,i+1].set_xlabel(labels[posplots[0,i]], fontsize=10)#, fontsize=20)
	...
	for j in range(dim-1, 0, -1):
		...
		xmax=np.ceil(np.max(xvals))
		step=(xmax-xmin)/Nticks
		axes[j,0].set_ylabel(labels[posplots[1,j]], fontsize=10)#, fontsize=20)
