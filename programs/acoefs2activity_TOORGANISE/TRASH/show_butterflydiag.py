...
def do_diagram(ax, date_interval, latitude, spot_area, latitude_range=[-90, 90], timezones = [], text_index=None):
	...
	if timezones != []:
		alpha=0.4
		for z in timezones:
			if z[2] != '': # If a color is specified...
	...
	for i in range(Nzones):
		...
		if rotate == False:
			...
			if date_filters[i][2] != '':
				...
				ax.plot(medfilt(collapsogram,smooth_lvl), -latitude, color='black',label=date_filters[i][0] + ' to ' + date_filters[i][1])
			...
			if date_filters[i][3] != False:
				...
				print('Median North {0:.0f}'.format(med_north))
				if i ==0:
					ax.annotate("{0:.0f}".format(med_south), xy=(0.188	, med_south), fontsize=8, va="center")
					print('Median South {0:.0f}:'.format(med_south))
...
def show_diagram():
	#file='/Users/obenomar/tmp/test_a2AR/tmp/External-data/Greenwich-data/daily_area.txt'
	file='/Users/obenomar/Work/tmp/test_a2AR/tmp/External-data/Greenwich-data/butterflydata.txt' #-1985-2016.txt'
	#
	fileout='/Users/obenomar/Work/tmp/test_a2AR/tmp/External-data/Greenwich-data/butterfly-fortalk.jpg'

	#date_filter_butterfly=['1985-01-01', '2022-01-01']
	date_filter_butterfly=['1967-01-01', '2022-01-01']
	#
	date_filters_projection=[]
	#date_filters_projection.append(['1967-01-01', '2022-01-01', '', False])
